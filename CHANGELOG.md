...
- **Known follow-up**: `entity.iso_std` for Glas is `PDB` (workbook value, not yet corrected to `Vienna-PDB`) — same manual fix flagged for prior imports in the old HOWTO; not yet applied, since `add_entity.py`/`backfill_from_csv.py` don't currently cover a single non-`sample_id`-keyed entity field correction (see `HOWTO_data_steward.md`, Workflow A).
